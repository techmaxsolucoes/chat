...
frappe.RealTimeClient = class RealTimeClient {
	constructor() {
		this.open_tasks = {};
	...
	init(host, port = 9000, lazy_connect = false) {
		if (frappe.boot.disable_async || !host) {
			return;
		}
        ...
        this.socket = io(this.get_host(host, port), {
            secure: window.location.protocol === 'https:' ? true : null,
